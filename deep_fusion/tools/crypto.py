...
def crypto_prices(
    ...
    dfs = pd.DataFrame(data.get("dictionary", []))
    ...
    return normalize_price_df(dfs, {
        ...
        "volume": "成交量",
        "amount": "成交额",
    }, source="okx", currency=currency, limit=limit, float_format="%.4f", date_unit=str, indicator_map={
        "macd": "MACD",
        "dif": "DIF",
        "dea": "DEA",
        "kdj_k": "KDJ.K",
...
def crypto_sentiment_metrics(
    ...
    loan_data = pd.DataFrame((loan_res.json() if loan_res else {}).get("dictionary", []))
    taker_data = pd.DataFrame((taker_res.json() if taker_res else {}).get("dictionary", []))
    if loan_data.empty and taker_data.empty:
        return format_error_csv("empty dictionary", "okx", fallback=symbol)
