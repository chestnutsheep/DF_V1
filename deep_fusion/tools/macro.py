...
_NBS_AVAILABLE = [
    "GDP", "CPI", "PPI", "INDUSTRIAL_VALUE_ADD",
    "INVENTORY", "FIXED_INVESTMENT", "UNEMPLOYMENT",
    "REAL_ESTATE_YOY",
]
...
def _fetch_with_priority(
    ...
    if df is None and nbs_fn and indicator in _NBS_AVAILABLE:
        try:
            periods, values = nbs_fn()
            if periods:
                df = pd.DataFrame({"日期": periods, "值": values})
                data_lake.store(indicator, df, source="nbs")
                source = "nbs"
...
def macro_growth(
    ...
    gdp, _ = _fetch_with_priority("GDP", ak.macro_china_gdp, nbs_fn=_nbs_gdp, limit=limit)
    ...
    ind, _ = _fetch_with_priority("INDUSTRIAL_VALUE_ADD", ak.macro_china_industrial_production_yoy,
                                  nbs_fn=_nbs_ind_yoy, limit=limit)
...
def macro_inflation(
    ...
    cpi, _ = _fetch_with_priority("CPI", ak.macro_china_cpi, nbs_fn=_nbs_cpi, limit=limit)
    ...
    ppi, _ = _fetch_with_priority("PPI", ak.macro_china_ppi, nbs_fn=_nbs_ppi, limit=limit)
...
def macro_monetary(
    ...
    unemp, _ = _fetch_with_priority("UNEMPLOYMENT", None, nbs_fn=_nbs_unemployment, limit=limit)
...
def macro_gdp(
    ...
    df, _ = _fetch_with_priority("GDP", ak.macro_china_gdp, nbs_fn=_nbs_gdp, limit=limit)
...
def macro_cpi(
    ...
    df, _ = _fetch_with_priority("CPI", ak.macro_china_cpi, nbs_fn=_nbs_cpi, limit=limit)
    if df is None or df.empty:
        return ""
    return df.to_csv(index=False, float_format="%.2f")
...
def macro_inventory_growth(
    ...
    df, _ = _fetch_with_priority("INVENTORY", None, nbs_fn=_nbs_inventory_yoy, limit=limit)
    if df is None or df.empty:
        return ""
    return df.to_csv(index=False, float_format="%.2f")
...
def macro_fixed_investment(
    ...
    df, _ = _fetch_with_priority("FIXED_INVESTMENT", None,
                                 nbs_fn=_nbs_fix_inv, limit=limit)
    if df is None or df.empty:
        return ""
    return df.to_csv(index=False, float_format="%.2f")
...
def _nbs_cpi():
    from ..tools.cycles import _fetch_nbs_cpi_yoy as _fn
    return _fn()


def _nbs_ppi():
    from ..tools.cycles import _fetch_nbs_ppi_yoy as _fn
    return _fn()


def _nbs_ind_yoy():
    from ..tools.cycles import _fetch_nbs_ind_yoy as _fn
    return _fn()


def _nbs_inventory_yoy():
    from ..tools.cycles import _fetch_nbs_inventory_yoy as _fn
    return _fn()


def _nbs_fix_inv():
    from ..tools.cycles import _fetch_nbs_fix_inv_monthly as _fn
    return _fn()


def _nbs_unemployment():
    from ..tools.cycles import _fetch_nbs_unemployment as _fn
    return _fn()
